...
test("game server executes upstream Game/gameServer and serves the upstream PixiJS client", async () => {
  ...
  assert.match(controlPlaneClient, /async addBots/);
  assert.match(controlPlaneClient, /async removeBots/);
  assert.match(adminUi, /className="remove-bots-button"/);
  assert.match(adminUi, /aria-label="목표 봇 수"/);
  assert.match(adminUi, /onSetBots\(target\)/);
  assert.match(adminUi, /controlPlaneClient\.setBots\(selectedRoom\.id, target\)/);
  assert.match(controlPlaneClient, /async setBots\(roomId: string, target: number\)/);
  assert.match(controlPlaneClient, /method: "PUT"/);
  assert.match(opsConsoleServer, /request\.method === "PUT"[\s\S]*botRunner\}\/bots\/target/);
  assert.match(botRunner, /new RoomBotTargets\(minimumBotsPerRoom\)/);
  assert.match(botRunner, /request\.method === "PUT" && pathname === "\/bots\/target"/);
  ...
  assert.match(botRunner, /selected\.length - \(roomId \? roomBotTargets\.get\(roomId\) : 0\)/);
