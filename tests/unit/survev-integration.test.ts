...
test("game server executes upstream Game/gameServer and serves the upstream PixiJS client", async () => {
  ...
  assert.match(roomMapPreviewStrip, /<LiveMapCanvas map=\{room\.mapLayout\} seed=\{room\.seed\} theme=\{room\.map\} \/>/);
  ...
  assert.match(adminCss, /\.room-map-tile \{[^}]*opacity: 0\.58;[^}]*filter: saturate\(0\.68\)/s);
  assert.match(adminCss, /\.room-map-tile:hover \{[^}]*opacity: 0\.92;[^}]*transform: translateY\(-5px\) scale\(1\.035\);/s);
  ...
  assert.doesNotMatch(roomMapPreviewStrip, /PlayerMarkers|currentPodName|roomPodLabel|podName|<span/);
  ...
  assert.match(roomCard, /\{bots\} BOTS/);
  ...
  assert.match(adminCss, /\.room-signal-graph strong \{[^}]*font: 900 86px/s);
  assert.match(adminCss, /\.room-load-line \{[^}]*stroke: rgb\(111 123 138 \/ 0\.88\);/s);
  assert.match(adminCss, /\.room-signal-graph rect\.is-warning-bar \{[^}]*fill: rgb\(255 159 10 \/ 0\.68\);/s);
  assert.match(adminCss, /\.room-signal-graph rect\.is-danger-bar \{[^}]*fill: rgb\(255 69 58 \/ 0\.72\);/s);
  assert.match(adminCss, /\.room-signal-graph > div \{[^}]*position: absolute;[^}]*left: 50%;[^}]*text-align: center;/s);
  ...
  assert.match(adminCss, /grid-template-areas:[^;]*"pod summary"[^;]*"pod counts"/s);
  ...
  assert.match(adminCss, /\.room-pod-identity dd \{[^}]*font: 950 clamp\(56px, 6vw, 96px\)/s);
  assert.match(adminCss, /\.room-pod-identity > div \{[^}]*min-height: 230px;/s);
  assert.match(adminCss, /\.room-card-actions \{[^}]*grid-template-columns: 1fr;[^}]*width: clamp\(150px, 15vw, 210px\);[^}]*margin: 8px 0 0 auto;/s);
...
test("five room Deployments, isolated canary, and registry discovery match the fleet contract", async () => {
  ...
  assert.equal((liveDeployments.match(/game\.opsia\.dev\/room-id: room-[0-4]/g) ?? []).length, 5);
