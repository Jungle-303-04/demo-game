...
test("game server executes upstream Game/gameServer and serves the upstream PixiJS client", async () => {
  ...
  assert.match(adminUi, /function admissionFailureRateForRoom/);
  assert.match(adminUi, /function admissionTone/);
  assert.match(adminUi, /function useAnimationFrameNow/);
  assert.match(adminUi, /window\.requestAnimationFrame\(update\)/);
  assert.match(adminUi, /function admissionToneLabel/);
  ...
  assert.match(serverBlock, /className=\{`server-block is-\$\{profile\.admissionTone\}`\}/);
  assert.match(serverBlock, /tickSurfaceStyleVars\(profile\.admissionHue, profile\.admissionRisk\)/);
  assert.match(serverBlock, /className="server-block-tick"/);
  assert.match(serverBlock, /profile\.admissionLabel/);
  assert.match(serverBlock, /profile\.admissionFailureRate/);
  ...
  assert.match(adminUi, /"장애 · 입장 실패율"/);
  assert.match(adminUi, /"주의 · 입장 실패율"/);
  assert.match(adminUi, /"정상 · 입장 실패율"/);
