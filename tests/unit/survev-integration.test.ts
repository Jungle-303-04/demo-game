...
test("game server executes upstream Game/gameServer and serves the upstream PixiJS client", async () => {
  ...
  assert.match(adminUi, /function roomMapUrl/);
  assert.match(adminUi, /url\.searchParams\.set\("view", "map"\)/);
  assert.doesNotMatch(adminUi, /function drawLiveMap|LiveMapCanvas|ResizeObserver/);
  ...
  assert.match(adminCss, /transition: transform 900ms linear/);
