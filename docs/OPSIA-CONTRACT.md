...
join-storm은 40 RPS부터 설정 가능한 단계로 증가하고 `opsia_sli_failure_ratio >= 0.2`가 두 구간
연속 관측된 최초 목표 RPS에서 유지된다. 각 합성 요청의 구조화 로그에는 상관관계 ID, 시나리오,
대상 room이 포함된다. 동일 버튼의 두 번째 클릭은 복구 판정과 무관하게 즉시 멱등 중단한다.
복구 완료 여부는 Kyro가 배포 diff와 포화 RPS 구간의 SLI·로그를 결합해 판단한다.
