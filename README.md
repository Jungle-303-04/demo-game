...
관리 API에는 bearer token을 요구하고 gateway는 HTTPS만 서비스한다. 실제 토큰과 인증서는 저장소에 커밋하지 않는다. overlay를 적용하기 전에 namespace와 세 Secret을 먼저 만든다.

```bash
kubectl --context cluster-2 apply -f deploy/k8s/base/namespace.yaml

kubectl --context cluster-2 -n sandbox create secret generic demo-game-admin \
  --from-literal=token='<32자 이상의 무작위 관리자 토큰>'

kubectl --context cluster-2 -n sandbox create secret generic demo-game-control \
  --from-literal=token='<32자 이상의 무작위 서비스 제어 토큰>'

kubectl --context cluster-2 -n sandbox create secret tls demo-game-tls \
  --cert=/absolute/path/to/tls.crt \
  --key=/absolute/path/to/tls.key

kubectl --context cluster-2 apply -k deploy/k8s/overlays/cluster-2
kubectl --context cluster-2 -n sandbox rollout status deployment/management-server
kubectl --context cluster-2 -n sandbox rollout status deployment/demo-game-gateway
...
브랜치를 push하는 것만으로 cluster-2 이미지가 자동 교체되지는 않는다. CI는 타입·lint·테스트·Kustomize 렌더링을 검증한다. 배포 담당자는 변경 commit으로 다섯 이미지를 build/push한 뒤 `deploy/k8s/overlays/cluster-2/kustomization.yaml`의 immutable tag를 갱신해야 한다.
...
base의 `stable` 태그도 항상 pull하도록 설정되어 있지만, cluster-2에는 commit 또는 release별 immutable tag 사용을 권장한다.
